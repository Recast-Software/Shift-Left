/*
Add Shift Left User Button
Updates the incident form to show the Shift Left button on the Incident form next to the Caller field.  
If the button already exists on the form, this will NOT add a new one.
*/
try {
	...
	var pTableName = 'incident';
	var pColumnName = 'caller_id';
	var pAttribute = 'x_471418_recast_ri_recast_user';
	var bUpdate = false;
	...
	if(gr.next()){
		...
		if (0 == nLength) {
			...
			if (-1 == n) {
				...
				gs.info("***SHIFT LEFT Add Add User Button attribute already exists");
			}
		}
		gs.info('4 updatedAttributes ' + updatedAttributes);
		gs.info('5 pDryRun ' + pDryRun);
		if (!pDryRun) {
			if (bUpdate) {
	...
	gs.info("***SHIFT LEFT Add User Button dictionary reference error: " + ex);
}
